...
export const MultiSelectCheckbox = <T extends FieldValues>({
    ...
    const onValueChange = (e: FormEvent<HTMLInputElement>) => {
        ...
        const setValue = 
        checked ? setCheckedValues(prev => [...prev,value]) : setCheckedValues(prev => prev.filter(v => v !== value))
            ...
            <Box className={classes.container}>
                ...
                <Box className={cx(classes.formGroup, className)}>
                    {options?.map(({label,value}: LabelValue) => {
                        return <label key={value} html-for={name + '-' + value}
                                    className={cx(classes.inputContainer)}>
                                <input className={classes.input}
                                    {...register(name as FieldValue<T>,
                                    { required: required })} type="checkbox"  value={label} onChange={onValueChange} name={value} id={name + '-' + value} />
